...
>
> 🟢 **AND UNLIKE `plan-circuit-model12.md`, BOTH DEVICES ARE PLUGGED IN.**
...
> line by line.
>
> **Evidence marks, the same set `plan-circuit-model12.md` uses:**
>
> | mark | means |
> |---|---|
...
depth, a MIDI map for each to the standard of `plan-circuit-model12.md`, power,
what each ADDS that the Circuit and the Model 12 do not already give, and where
each lands in `plan-device-layouts.md`'s fourteen control types. Plus a list of
...
- **And it needs no new control type.** Everything it can send is already in
  `plan-device-layouts.md`'s catalogue, and every one of that catalogue's four
...
demo/rack/index.html:83    ... ({ sampleRate: RATE, latencyHint: 'interactive' });
...
the fourth state `plan-device-layouts.md` §9.1 names: plugged in, but is it the
...
at all**, in either direction. `plan-device-layouts.md` T9 records the same gap
...
SETTING.** `plan-device-layouts.md` T7 is about exactly this distinction and
...
🔴 **SO `plan-device-layouts.md` §5.3's RULE HOLDS AND GETS SHARPER.** That
...
`plan-device-layouts.md` T8, *"a lamp, which is a control that READS"*, and T13,
...
`plan-circuit-model12.md` §7's argument about who owns the tempo. It cannot be a
clock source and it cannot follow one. `plan-device-layouts.md` T14 refuses
...
its own PSU. 📄 DOC, `plan-circuit-model12.md` §3.10, the Circuit's user guide:
...
3. **It is class compliant at full speed and USB 1.1**, which is the most
   ...
   interfaces, and `plan-circuit-model12.md` §6.8 has a deferred Pi stage that
...
`plan-circuit-model12.md` §6.11 establishes it from TASCAM's own manual: the
...
1. 🔴 **THE MODEL 12'S DIN OUT IS NOT CLEAN.** `plan-circuit-model12.md` §6.11
...
`/rack/` are both keyboard pages: `/rack/` plays Ableton Live from a browser and
...
`plan-circuit-model12.md` §5's direction table gains a third row and it is
entirely one way.
...
📁 **REPO**, `plan-device-layouts.md` §2's catalogue.
...
`plan-device-layouts.md` T9 already records *"nothing sends one"*. **This device
...
instead is a **transport**, in the sense of `plan-device-layouts.md` §5.3's
...
bound to a DIN route can never confirm anything. `plan-device-layouts.md` §9.3
...
`plan-circuit-model12.md` §7 is an argument about who owns the tempo, and it
...
`plan-device-layouts.md` T7's entire distinction, measured rather than assumed.
...
- 🔴 **Do not put an MK-425C layout in `.pos-controls`.** 📁 REPO,
  `plan-device-layouts.md` §5.4: `demo/verify.mjs` presses every
...
- `demo/shell/midi.mjs`, `demo/shell/cc-adapter.mjs`, `demo/shell/board.mjs`,
  `demo/shell/keyboard.mjs`, `demo/rack/index.html`, `demo/grains/index.html`
- `measured-devices-2026-09-20.md`, `plan-circuit-model12.md`,
  `plan-device-layouts.md`, `CLAUDE.md`
