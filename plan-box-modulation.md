# plan-box-modulation — the granulator's movement, on the board
...
>
> Everything else is read off `rig/box/norns/Engine_Pappus.sc`,
> `rig/box/pappus.mjs`, `rig/box/box.mjs`, `rig/box/norns/run-pappus.scd` and
...
| evidence | reading |
|---|---|
| `/etc/default/positron-box` | `PAPPUS_TINY=1`, with a comment dating the decision to 2026-09-13 and naming the price |
| the live process environments — `box.mjs` pid 176447, `sclang` pid 176615, `scsynth` pid 176653 | all three carry `PAPPUS_TINY=1` |
| `journalctl -u positron-box` | `Engine_Pappus: TINY graph`, Sep 13 22:47:31 — the same minute pid 176447 started |
...
file on disk says more than that.** `/opt/positron-box/rig/box/norns/run-pappus.scd`
...
```
/opt/positron-box/rig/box/pappus.mjs                       dc595efaa1e152aa425a003850f61eaf
/opt/positron-box/rig/box/box.mjs                          448f04ab15f01351a9ce74093cd52ab2
/opt/positron-box/rig/box/norns/run-pappus.scd             3b0095ff8bcdfef73aee9678232333d8
...
`rig/box/pappus.mjs:54` fixes the port and address; `run-pappus.scd`'s
...
process CPU      scsynth 23.7 %   box.mjs 5.1 %   sclang 0.9 %   jackd 0.3 %
...
`rig/box/pappus.mjs:376` is `DRIFT`: six parameters, each a pair of sines at
...
|---|---|---|---|
| **A · node** | `pappus-mod.mjs` in `box.mjs` | `/pappus/cmd` → sclang → `synth.set` → `/n_set` | one JACK period, **21.3 ms** |
...
`plan-radio-patches.md §1.6` has the table; all five are compiled on TINY. The
...
- `params.set` from `/grains/` — one per knob, per drag (`box.mjs:1072`);
...
1. 🔴 **ONE MODULATOR, TWO MACHINES — and the test already exists.**
   ...
   `plan-twins`' subject exactly, and `rig/box/pappus.mjs:37` already carries
...
`plan-radio-patches.md §1.5`) and of `createEnvFollower`'s:
...
3. **Node measures it itself.** The board already captures audio for the relay;
   an rms per frame is nearly free in `box.mjs`. ⚠️ But that measures what is
...
with citations (the full table is `plan-radio-sound.md §1`, and it is correct as
of this reading):
...
nothing will say so.** `/etc/default/positron-box` carries a 🔴 warning against
...
`plan-radio-sound.md §① and §⑥` derive the same arithmetic for the tab.
...
`plan-radio-patches.md §2.1`: *"The page used to hand you a random number and
...
parameter and grade the sound"* (`box.mjs:1064`). A harness has to be able to
...
`box.mjs:1053` already says so about `notes`. `sends` is what the box sent.
...
Two things the spec cannot do, which need their own guards in `box.mjs`:
...
(`box.mjs:942`). ✅ It is running now. A 25 Hz modulator on any of those six
...
`box.mjs:930–938` records `pappus-live.mjs` switching the drift off, rolling ten
...
3. **An expiry, like `grain.report` has.** `box.mjs:969` keeps shipping grains
...
1. **A read-only measurement module in `rig/box/`** — one UDP socket to
...
6. `params.mod` and `params.patch` on `box.mjs`, the `mod:` block on
