...
| **`~/moq-rs-draft14/`** | 1.2 GB; `target/release/moq-relay-ietf`, 18 MB, built 2026-09-10 | the LAN MoQ relay — the measured 58 ms key→ear path. Not needed for `/rack/`, only for playing. | no — a Rust build of a third-party tree | yes |
...
- **The 1.2 GB Rust relay tree is not needed for `/rack/` at all.** It belongs
...
not the goal here: `/rack/` asks nine yes/no questions and `play.html` sends
notes.
...
exactly this. It would make `/rack/`'s ~20 OSC round trips near-instant instead
...
- **Do not make this an always-on service in the box's sense**, even if item 3
  ...
  can put up a crash-recovery dialog and refuse an AppleScript `quit`. `/rack/`
