#!/usr/bin/env node
// P3A media-under-bulk driver (clone of run-heavy.mjs; changes: BASE :8894,
// udd prefix m2m-p3a-udd, + kind:"egress" transport-bytes sampling every 30 s
// for the plan §4 cost-model validation). Original comment follows.
...
const UDD_BASE = "/private/tmp/claude-501/-Users-s32863-personal-elektron/3e55abee-40f5-4628-b7fd-775f7a2bfd0b/scratchpad/m2m-p3a-udd";
...
// transport-level byte counters straight off the page's PeerConnection (room.html
// declares top-level `let pc` — visible to evaluate as a global lexical binding).
// wire bytes = DTLS+RTP+RTCP as seen by the client; inbound wire ≈ CF billable egress.
const egressGrab = async () => {
  if (typeof pc === "undefined" || !pc) return null;
  const st = await pc.getStats();
  const o = { tPage: performance.timeOrigin + performance.now(),
              wireSent: null, wireRecv: null, pktSent: null, pktRecv: null,
              outV: 0, outA: 0, inV: 0, inA: 0, outVRetrans: 0,
              outVHeader: 0, inVHeader: 0, inAHeader: 0, outAHeader: 0, nInV: 0 };
  st.forEach(s => {
    if (s.type === "transport") {
      o.wireSent = s.bytesSent; o.wireRecv = s.bytesReceived;
      o.pktSent = s.packetsSent; o.pktRecv = s.packetsReceived;
    }
    if (s.type === "outbound-rtp") {
      if (s.kind === "video") { o.outV += s.bytesSent || 0; o.outVRetrans += s.retransmittedBytesSent || 0; o.outVHeader += s.headerBytesSent || 0; }
      else { o.outA += s.bytesSent || 0; o.outAHeader += s.headerBytesSent || 0; }
    }
    if (s.type === "inbound-rtp") {
      if (s.kind === "video") { o.inV += s.bytesReceived || 0; o.inVHeader += s.headerBytesReceived || 0; o.nInV++; }
      else { o.inA += s.bytesReceived || 0; o.inAHeader += s.headerBytesReceived || 0; }
    }
  });
  return o;
};
...
const run = async () => {
  ...
  if (!ro || !ro.ok) { console.error("server on :8894 not reachable — start server.py first"); process.exit(1); }
  ...
  // ---- measurement window --------------------------------------------------
  const tRun = Date.now();
  let lastCpu = 0, lastEgress = 0;
  const sampleEgress = async (tag) => {
    const rows = await Promise.all(pages.map(async p => {
      const e = await p.page.evaluate(egressGrab).catch(() => null);
      return e ? { id: p.id, cls: classFor(p.id), ...e } : null;
    }));
    await collect({ kind: "egress", n: N, t: Date.now(), label: LABEL, tag,
                    rows: rows.filter(Boolean) });
  };
  await sampleEgress("window-start");
  while (Date.now() - tRun < DURATION_S * 1000) {
    await new Promise(r => setTimeout(r, 5000));
    const states = await Promise.all(pages.map(p => p.page.evaluate(grab)));
    say(describe(states));
    if (Date.now() - lastEgress > 30000) { lastEgress = Date.now(); await sampleEgress("mid"); }
