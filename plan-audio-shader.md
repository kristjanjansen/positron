...
> source in particular: `plan-plate.md §0` records that
...
`plan-visuals.md` (which side of the wire a picture is made on, and the GPU
...
`plan-visuals.md §2.2` states the constraint: a fragment shader writes exactly
...
gone** (`plan-plate.md §0`). They are the best numbers available and none of them
...
- image load and store, shader storage blocks, `atomic_uint`, and anything
  `compute`. ES 3.10 only, and `plan-visuals.md §2.2` records that none of them
...
- 🔴 **Never sample the target being drawn to.** `plan-visuals.md §2.3` quotes
...
2. 🔴 **`drain_stdin` reads ONE BYTE PER `read()` SYSCALL.** READ
   ...
   payload.
3. **The board's texture limit is not the issue.** READ `plan-visuals.md §3.3`,
   measured there: `GL_MAX_TEXTURE_SIZE 4096`. A 64x4 texture is not close to
   anything.

### 5.3 The Quest

READ `research/quest-xr-2026-09.md` via `plan-visuals.md §2.4` and CLAUDE.md:
...
|---|---|---|---|
| Raspberry Pi 4, V3D 4.2.14.0 | kaleido fold, 3 octaves of noise, warp, feedback, 1 pass, 1280x720 | **18.31 ms, 54.6 fps, 50.3 Mpix/s** | `plan-visuals.md §3.4`, measured there |
...
| laptop, ANGLE Metal, M2 Pro | the same shader, 720p | **0.66 ms, 1392.1 Mpix/s** | `plan-visuals.md §5.1`, measured there |
...
nothing in this repo has measured one.** `plan-visuals.md §5.6` names it as an
...
as free"*. READ `plan-visuals.md §2.6`: it is raw nanoseconds and effectively
...
(`plan-plate.md §0`). A page that hard-codes `-40.0 .. -14.5 LUFS` and
...
- 🔴 **Sub-bass as the primary gesture.** MEASURED, research §1.4: 20 to 60 Hz is
  **1.8% of total energy** and has the **narrowest** dynamic range of any band at
  ...
  signal in the piece.
- **Compute shaders and WebGPU.** `plan-visuals.md §2.5`: WebGL2 is the renderer,
...
**10.5 Nothing here has measured a phone or a headset.** `plan-visuals.md §5.6`
