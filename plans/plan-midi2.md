...
Companion to `plans/plan-controller.md` (the 7-bit ceiling, and the SuperCollider voice
that escapes it), `plans/plan-circuit-model12.md` §6.4 (Web MIDI and the one line that
blocks SysEx), `proto/osc/NOTES.md` (the codec, the atomicity argument and the
transport tables), `plans/plan-ws.md` and `demo/shell/wire.mjs` (the envelope), and
`plans/plan-nodes.md` (the tagging convention below).
...
`{sysex: false}`, and `plans/plan-circuit-model12.md` §6.4 already records that every
...
- **Property Exchange**: **JSON carried over SysEx**, so a device can hand over
  ...
  `plans/plan-circuit-model12.md`'s.
...
`plans/plan-board-modulation.md:275`: the granular engine sets `lagt = 0.02` and runs
...
**Where it does.** `plans/plan-controller.md` §7: *"128 steps across a wide cutoff
...
not available on that instrument at all, because `plans/plan-controller.md` §7
...
`plans/plan-circuit-model12.md` §6.4 already records that SysEx is the larger
...
**Measured previously and not re-run** (`plans/plan-ws.md`, `demo/perf-wire.mjs`): the
...
from "the network is slow today". `plans/plan-ws.md` measured **fan-out to 15
...
AudioWorklet and in node on the Pi, and `plans/plan-controller.md` §2.3 already
...
send". `plans/plan-circuit-model12.md` §6.3 already found a second defect in the same
...
worth having on their own, and `plans/plan-nodes.md` §2.2 already names that as the
...
- **Do not add routing to the relay.** `plans/plan-nodes.md` §2.2 already refused this
...
- **Do not turn SysEx on as a side effect.** It is a strictly larger permission
  than notes and `plans/plan-circuit-model12.md` §6.4 already says it should be a
...
- **Chrome on a phone, and Chrome on the Pi.** `plans/plan-hardware.md` §6 already
