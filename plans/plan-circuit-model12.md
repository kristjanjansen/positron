...
It is also exactly the demonstration `plans/plan-controller.md` was written to build,
...
(`bankCC 32`, `rootCC 0`, read from `voices.list` in `plans/plan-controller.md` §1.2),
...
|---|---|---|
| `rig/m1/live-agent.mjs` | relay socket, notes out, audio back | yes, `/able/` |
...
⚠️ **1.4 USES `/instrument/` ON PURPOSE AND NOT `/able/`.** 📁 REPO:
`/instrument/` reads `requestMIDIAccess` behind a button and **sends nothing to
any relay**. `/able/` is the only page that calls `createMidi`, and it also
...
- `demo/shell/cc-adapter.mjs`, `demo/shell/midi.mjs`, `demo/shell/hardware.mjs`,
  `demo/shell/wire.mjs`, `demo/instrument/index.html`, `demo/able/index.html`
...
- `plans/plan-hardware.md` §8, `plans/plan-controller.md`, `plans/plan-portable-board.md`,
  `plans/plan-rack.md`
