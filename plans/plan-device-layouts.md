...
> and the Tascam Model 12 comes from `plans/plan-circuit-model12.md`, which read them
...
>
> **Evidence marks, the same ones `plans/plan-circuit-model12.md` uses:**
>
> | mark | means |
> |---|---|
> | 📄 **DOC** | out of a manufacturer PDF. Quoted or cited through `plans/plan-circuit-model12.md`. |
...
⚖️ COUNTED from `plans/plan-circuit-model12.md` §3.3, §3.4, §3.5 and §3.6: 52 CC
...
**Which parameters.** 📄 DOC, `plans/plan-circuit-model12.md` §3.3: portamento rate,
...
form: 🌐 THIRD PARTY, `plans/plan-circuit-model12.md` §4.4, MCU V-Pots are CC 16 to
...
`plans/plan-circuit-model12.md` documents no per pad MIDI address for any of them.
...
`plans/plan-circuit-model12.md` §7 rather than here.
...
🌐 THIRD PARTY, `plans/plan-circuit-model12.md` §4.4, from a published reverse
...
`plans/plan-circuit-model12.md` §6.5, and it is worse in a specific way. There, two
...
for a channel to go.** This reproduces `plans/plan-circuit-model12.md` §6.5's
...
is this type with the ambiguity removed. 📁 REPO, `plans/plan-midi2.md`: Chrome
...
`plans/plan-circuit-model12.md` §4.4 notes that TASCAM's own manual prints the DAW
...
5. **Is `makeCcSend` repaired to key by `${ch}:${controller}`?** §4.2 E measures
   the defect and `plans/plan-circuit-model12.md` §6.5 measures it independently.
