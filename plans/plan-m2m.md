...
Companion to `plans/plan.md` (one-to-many ground truth — this file does not restate it)

Provenance, same convention as `plans/plan.md`:

- ✅ **measured** — measured in this repo (see `plans/plan.md` / `PROGRESS.md` / `rig/`)
...
`plans/plan.md` (LL-HLS 2.4–4 s tuned ✅, WHIP→WHEP 74 ms p50 ✅, cues DO relay 27 ms one-way ✅).
...
5. **Graceful per-participant degradation** — one dropped participant costs their tiles only.
   Contrast: ✅ the one-to-many broadcast dies on encoder socket close (`plans/plan.md` §10); the grid must
...
- **The repo has already measured this SFU without knowing it.** 📄 Stream's WHIP/WHEP was migrated
  ...
  WHIP→WHEP measurement in `plans/plan.md` §2.2 therefore traversed this exact media plane.
...
- ✅ The publish URL is the credential (`plans/plan.md` §2.2) — distributing per-audience-member publish
...
✅ 17.9 ms one-way relay proven (draft-14, `plans/plan.md` §2.3) — the best transport number in the repo.
...
```
                        STAGE (one-to-many, plans/plan.md, unchanged)
...
| Stage pipeline: ingest, LL-HLS player v5, WHEP, relay/OBS | ✅ built + chaos-tested (`plans/plan.md`) |
...
- **Leave detection inverts the socket-close lesson**: in the one-to-many world a closed socket
  kills the broadcast (✅ `plans/plan.md` §10, the hard-won finding); here `webSocketClose` on the DO is
...
- **Reconnect** = new SFU session ⇒ new sessionId ⇒ re-`publish`; subscribers treat it as
  ...
  from a fresh pull. (Rebuild-never-patch, ✅ design rule from `plans/plan.md` §10, applied to signaling.)
...
- **Two different Cloudflare products, two different meters — do not mix them.** This table is
  ...
  by recording and by the storage cap — never by the delivery rate. See `plans/plan-session.md` §5.
...
5. **Browser matrix** ⚠️ — Safari/Firefox WebRTC is fine for plain pub/sub 📄, but rid-based
   ...
   discipline applies; ✅ hidden-tab trap rules from `plans/plan.md` §4.2 carry over verbatim —
...
6. **Timing measurement** — ✅ abs-capture-time is refused by this infrastructure at negotiation
   (`plans/plan.md` §2.2), so grid latency claims must again be burned-pixel measured; the WHEP rig
...
7. **Secret custody** — the app secret is a full-control credential 📄; it must live only in the
   proxy Worker. ⚠️ And the account API token still needs rotation (`plans/plan.md` §6) — same hygiene
   moment, do both.
...
- Chaos, ported from `plans/plan.md` discipline: kill a participant's network mid-publish (blast radius
