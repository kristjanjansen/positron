...
> Extends `plans/plan-xr.md`, which holds the device measurements and the five defects
...
page hands sound to a `<video>` element and loses it, while `/keys/` and `/able/`
...
⚠️ It also needs the doff watchdog from `plans/plan-xr.md` §7.7 **first**, not after.
...
5. **A doff stops the meter.** Not built. `plans/plan-xr.md` §7.7.
