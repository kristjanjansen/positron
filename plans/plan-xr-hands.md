...
> in the future."*
>
> Extends `plans/plan-xr.md` (the device measurements and the five defects the headset
> found) and `plans/plan-xr-room.md` (the room, and §3 "Interactivity", which says
...
`plans/plan-xr-room.md` §5.6's rule ("a panel that is not live says so in words") and
...
2. **The left one ends your session.** A page cannot prevent it, and
   `plans/plan-xr.md`'s rule that the page must own a way out does not extend to owning
...
`plans/plan-xr-room.md` §3: **"One controller. Measured, in this user's hands. Every
...
**Legibility is unmeasured and it is the real risk.** `plans/plan-xr-room.md` §9 lists
...
`plans/plan-xr-room.md` §3 already names press feedback as the missing thing, and
...
(`plans/plan-xr-room.md` §9). The tablet at §5.2's 1024×640 is **64% of those pixels**,
...
the second covers steady state, and `plans/plan-xr-room.md` §9 already found those two
...
5. **A 0.20 m × 0.13 m quad is legible.** UNMEASURED — and `plans/plan-xr-room.md` §9
...
**`plans/plan-xr-room.md` §3 says "pointing at a panel and pressing a button is not
