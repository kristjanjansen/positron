...
# Namespace: positron-shim-test. One-clock g2g stays valid (burn happens on host).
cd "$(dirname "$0")"
exec python3 gen-frames.py | ffmpeg -hide_banner -loglevel warning \
  ...
  | docker run -i --rm --name moq-mtx-cfpub -v moq-target14:/target moq-dev \
    /target/release/moq-pub --name positron-shim-test "https://draft-14.cloudflare.mediaoverquic.com"
