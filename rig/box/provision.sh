#!/bin/bash
# rig/box/provision.sh — find the box on the LAN and set it up, from here.
#
#   ./provision.sh              sweep the subnet for it
#   ./provision.sh 192.168.1.57 go straight to it
#
# Why a sweep rather than `positron-box.local`: mDNS is multicast UDP and does
...
# resolution that may or may not exist, ask every address whether it is the box.
set -uo pipefail
...
find_box() {
  ...
  echo "sweeping ${base}.0/24 for a host that answers as the box ..." >&2
  ...
  for ip in $(arp -an | grep -oE '\(([0-9]{1,3}\.){3}[0-9]{1,3}\)' | tr -d '()' | sort -u); do
    ...
    # A 2 s budget each: the box answers instantly, everything else times out.
...
echo "== copying the box"
# Ship rig/box plus every module it imports, at the paths it expects. tar over
...
echo "== setup (needs sudo on the box)"
ssh -t "$USER_@$IP" 'cd ~/positron/rig/box && sudo ROOM=${ROOM:-studio-1} ./setup.sh'

echo
echo "== checks that need the board, which nothing here could fake"
ssh "$USER_@$IP" 'cd ~/positron/rig/box && node test.mjs 2>&1 | tail -3; echo; node bench.mjs'
echo
