// rig/board/yoshimi.mjs — read Yoshimi's instrument library out of Yoshimi's own
...
export const YOSHIMI_DIR = process.env.BOARD_YOSHIMI_CONFIG || `${homedir()}/.config/yoshimi`;
 ...
 * parser: the shape is fixed, flat and machine-written, and a dependency on an XML
 * library is a dependency to install on the board.
 */
...
/**
 * The whole answer, for one board: which banks exist, what is in them, and what
