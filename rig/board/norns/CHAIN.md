...
else 0 — so out of the board both granulators enter at RESONATOR and run the
...
the board**, and silent for every patch written on any other rung — LESSONS #62's
...
boards on SIGNAL do not exist, and **a meter reading zero because a stage was
...
comparing two different builds.
    ...
    #    nothing yet
    ./rig/board/push.sh --no-restart            # or: ./rig/board/push.sh <ip>
    ...
    ssh positron@$BOX 'sudo systemctl stop positron-board; \
...
```
    ...
    ssh positron@$BOX 'sudo systemctl start positron-board; sleep 3; \
      systemctl is-active positron-board'
