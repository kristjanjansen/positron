// rig/board/yoshimi-test.mjs — drive a board running Yoshimi and check that its
...
ws.onmessage = (e) => {
  if (typeof e.data !== 'string') {
    ...
    // The board restarts its sequence when the source changes, so a step BACK is
...
ws.onopen = async () => {
  try {
    ...
    ok('board.ping -> board.pong', !!(await answer(send({ type: 'board.ping' }), 'board.pong')));
    ...
    // slots are NOT a run. A list that looked like either would mean the board
