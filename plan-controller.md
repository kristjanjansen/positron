...
- `demo/shell/wire.mjs:35-40` exports `LIMITS` as `maxBytes 256 KiB`,
  ...
  **16.7x on message rate** and **8x on sockets**. A page that gated its send
  ...
  Already reported in `plan-radio-messages.md` and not fixed.
...
| leg | figure | source |
|---|---|---|
| runtime `ping` to `pong`, DO never woken | p50 26.0 / 36.4 / 37.5 ms across 3 runs | `plan-ws.md`, `demo/perf-wire.mjs` |
